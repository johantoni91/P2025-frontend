	...
	@foreach ($data["role"] as $roles)
		<div class="hidden rounded-lg bg-gray-50 p-4 dark:bg-gray-800"
			...
			<div class="flex items-center justify-end p-3">
				@php
					$face = array_filter($data["access"], function ($val) use ($roles) {
					    return $val["roles_id"] == $roles["id"];
					});
				@endphp
				<a class="flex items-center justify-start gap-3"
					href="{{ route("role.face", [current($face)["roles_id"]]) }}"><img
					...
					<tbody>
						@foreach ($data["modul"] as $i)
							...
							<form action="{{ route("role.update", [$roles["id"]]) }}"
								...
								@csrf
								@method("PUT")
								@if (current($get_access)["module"]["route"] != "dashboard")
											...
											<div class="flex items-center justify-center gap-2">
												<div class="flex items-center">
													<input
														...
														type="radio"
														value="0"
														{{ current($get_access)["graph"] == "0" ? "checked" : "" }}>
													<label class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300"
														for="graph_{{ $i["id"] }}_1">Nonaktif</label>
												</div>
												<div class="flex items-center">
													<input
														class="h-4 w-4 border-gray-300 bg-gray-100 text-blue-600 focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:ring-offset-gray-800 dark:focus:ring-blue-600"
														...
														type="radio"
														value="1"
														{{ current($get_access)["graph"] == "1" ? "checked" : "" }}>
													<label class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300"
														for="graph_{{ $i["id"] }}_2">Aktif</label>
												</div>
											</div>
										</td>
										<td class="px-6 py-4">
