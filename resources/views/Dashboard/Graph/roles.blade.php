<div class="w-auto max-w-sm rounded-lg bg-white shadow-lg dark:bg-gray-800 dark:shadow-blue-500">
	<div class="flex justify-between">
		<div class="w-full px-2 py-3 md:py-5">
			<h5 class="mb-2 ms-5 text-2xl font-semibold leading-none text-gray-900 dark:text-white">Roles</h5>
		</div>
	...
	<div class="grid grid-cols-1 items-center justify-between border-t border-gray-200 dark:border-gray-700">
		<div class="p-5">
			<span class="text-sm font-semibold">Total Role : {{ $data["role"] }}</span>
		</div>
	</div>
</div>
<script>
	const roleOptions = {
		...
		xaxis: {
			categories: @json($role["categories"]),
			labels: {
				show: false,
			},
			axisBorder: {
	...
	const role = new ApexCharts(document.getElementById("roles-chart"), roleOptions);
	role.render();
</script>
