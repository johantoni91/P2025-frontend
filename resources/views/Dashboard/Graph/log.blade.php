<div class="w-auto max-w-sm rounded-lg bg-white shadow-lg dark:bg-gray-800 dark:shadow-blue-500">
	...
	<div class="grid grid-cols-1 items-center justify-between border-t border-gray-200 dark:border-gray-700">
		<div class="p-5">
			<span class="text-sm font-semibold">Total Aktivitas : {{ $data["logs"] }}</span>
		</div>
	</div>
</div>
<script>
	const logOptions = {
		...
		xaxis: {
			categories: @json($log["categories"]),
			labels: {
				show: false,
			},
			axisBorder: {
	...
	const logs = new ApexCharts(document.getElementById("log-chart"), logOptions);
	logs.render();
</script>
